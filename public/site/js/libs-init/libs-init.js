...
$(document).ready(function () {
	...
	if (topUserSearch.length) {
		topUserSearch.selectize({
			...
			searchField: ['name'],
			options: [
				{image: 'site/img/avatar30-sm.jpg', name: 'Marie Claire Stevens', message:'12 Friends in Common', icon:'olymp-happy-face-icon'},
				{image: 'site/img/avatar54-sm.jpg', name: 'Marie Davidson', message:'4 Friends in Common', icon:'olymp-happy-face-icon'},
				{image: 'site/img/avatar49-sm.jpg', name: 'Marina Polson', message:'Mutual Friend: Mathilda Brinker', icon:'olymp-happy-face-icon'},
				{image: 'site/img/avatar36-sm.jpg', name: 'Ann Marie Gibson', message:'New York, NY', icon:'olymp-happy-face-icon'},
				{image: 'site/img/avatar22-sm.jpg', name: 'Dave Marinara', message:'8 Friends in Common', icon:'olymp-happy-face-icon'},
				{image: 'site/img/avatar41-sm.jpg', name: 'The Marina Bar', message:'Restaurant / Bar', icon:'olymp-star-icon'}
			],
			render: {
				option: function(item, escape) {
					return '<div class="inline-items">' +
						...
						'</div>'+
						(item.icon ? '<span class="notification-icon"><svg class="' + escape(item.icon) + '"><use xlink:href="site/icons/icons.svg#' + escape(item.icon) + '"></use></svg></span>' : '') +
						'</div>';
				},
	...
	init: function () {
		...
		for (var key in this.maps) {
			...
			data.markers.forEach(function (item) {
				data.config.icon['iconUrl'] = './site/img/' + item.icon;
