...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch_user = User::where('id', $request['id'])->first();
            $fetch = array($fetch_user);
        }

        if ($request['type'] == 'group') {
            $fetch_group = Group::where('group_id', $request['group_id'])->first();
            $fetch = array($fetch_group);
        }
        // send the response
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('/storage/' . config('chatify.user_avatar.folder') . '/' . isset($fetch[0]['avatar']) ? $fetch[0]['avatar'] : $fetch[0]['group_avatar']),
        ]);
    }
        ...
        if(isset($request['type'])){
            if($request['type'] == "group"){
                ...
                if (!$error_msg) {
                    ...
                    $messageData = Chatify::fetchMessageGroup($messageID);
        ...
        if ($query->count() > 0) {
            foreach ($messages as $message) {
                $allMessages .= Chatify::messageCard(
                    Chatify::fetchMessage($message->id) // id of table messages
                );
            }
            // send the response
            return Response::json([
    ...
    public function fetchMessagesGroup(Request $request)
    {
        // messages variable
        $allMessagesGroup = null;

        // fetch messages
        $query = Chatify::fetchMessagesGroupQuery($request['id'])->orderBy('created_at', 'asc');
        ...
        if ($query->count() > 0) {
            foreach ($messages as $message) {
                $allMessagesGroup .= Chatify::messageCard(Chatify::fetchMessageGroup($message->message_group_id));
            }
            // send the response
            return Response::json([
                'count' => $query->count(),
                'messages_group' => $allMessagesGroup,
            ]);
        }
        // send the response
        return Response::json([
            'count' => $query->count(),
            'messages_group' => '<p class="message-hint"><span>Say \'hi\' and start messaging</span></p>',
        ]);
    }
    /**
    ...
    // TODO: GROUP
    public function seenGroup(Request $request)
    {
        // make as seen
        $seen = Chatify::makeSeenGroup($request['id']);
        // send the response
        return Response::json([
            'status' => $seen,
        ], 200);
    ...
    {
        $groups = MessageGroup::join('groups', function ($join) {
            $join->on('message_groups.from_id', '=', 'groups.group_id')
                ->orOn('message_groups.to_id', '=', 'groups.group_id');
        })
            ->where('message_groups.from_id', Auth::user()->id)
            ->orWhere('message_groups.to_id', Auth::user()->id)
        ...
        if ($groups->count() > 0) {
            ...
            foreach ($groups as $group) {
                if ($group->message_group_id != Auth::user()->id) {
                    // Get user data
                    $groupCollection = Group::where('group_id', $group->group_id)->first();
                    $contacts .= Chatify::getContactGroupItem($request['messenger_id'], $groupCollection);
                }
            }
        }

        // send the response
        return Response::json(['contacts' => $groups->count() > 0 ? $contacts : '<br><p class="message-hint"><span>Your contatct list is empty</span></p>',], 200);
    }
    /**
    ...
    public function search(Request $request)
    {
        ...
        $records_group = Group::where('group_name', 'LIKE', "%{$input}%");
        // $records = User::with('groups')->whereHas('groups', function ($q) use ($input) {
        //     $q->where('group_name', 'LIKE', "%{$input}%");
        ...
        foreach ($records_group->get() as $recordItem) {
            $getRecords .= view('Chatify::layouts.listItem', [
                'get' => 'search_item_group',
                'type' => 'group',
                'group' => $recordItem,
            ])->render();
        }
        // send the response
        return Response::json([
            'records' => ($records->count() > 0 || $records_group->count() > 0)
                ? $getRecords
