...
class ChatifyMessenger
{
    ...
    public function fetchMessageGroup($id)
    {
        $attachment = $attachment_type = $attachment_title = null;
        // $msg = MessageGroup::where('message_group_id', $id)->first();
        $msg = MessageGroup::where('message_group_id', $id)->first();
     ...
     * @return Collection
     */
    public function fetchMessagesGroupQuery($group_id)
    {
        // return MessageGroup::where('from_id', Auth::user()->id)->orWhere('to_id', $group_id)
        //     ->orWhere('from_id', $group_id)->where('to_id', Auth::user()->id);
        return MessageGroup::where('to_id', $group_id);
    }
    /**
    ...
    // TODO: GROUP
    public function makeSeenGroup($group_id)
    {
        MessageGroup::Where('from_id', $group_id)
            ->where('to_id', Auth::user()->id)
            ->where('seen', 0)
            ->update(['seen' => 1]);
        return 1;
    ...
    // TODO: GROUP
    public function getLastMessageGroupQuery($group_id)
    {
        return self::fetchMessagesGroupQuery($group_id)->orderBy('created_at', 'DESC')->latest()->first();
    }
    /**
    ...
    // TODO: GROUP
    public function countUnseenGroupMessages($group_id)
    {
        return MessageGroup::where('from_id', $group_id)->where('to_id', Auth::user()->id)->where('seen', 0)->count();
    }
    /**
     ...
     * @return void
     */
    public function getContactGroupItem($messenger_id, $group)
    {
        // get last message
        $lastMessage = self::getLastMessageGroupQuery($group->group_id);

        // Get Unseen messages counter
        $unseenCounter = self::countUnseenGroupMessages($group->group_id);
