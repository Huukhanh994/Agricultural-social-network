...
@if($get == 'groups')
<table class="messenger-list-item @if($group->group_id == $id && $id != "0") m-list-active @endif"
    data-contact="{{ $group->group_id }}">
    <tr data-action="0">
        {{-- Avatar side --}}
        <td style="position: relative">
            <span class="activeStatus"></span>
            <div class="avatar av-m"
                style="background-image: url('{{ asset('/storage/group_avatar/'.$group->group_avatar) }}');">
            </div>
        </td>
        {{-- center side --}}
        <td>
            <p data-id="{{ $type.'_'.$group->group_id }}">
                {{ strlen($group->group_name) > 25 ? trim(substr($group->group_name,0,25)).'..' : $group->group_name }}
...
@if($get == 'search_item_group')
<table class="messenger-list-item" data-contact="{{ $group->group_id }}">
    <tr data-action="0">
        {{-- Avatar side --}}
        <td>
            <div class="avatar av-m"
        ...
        <td>
            <p data-id="{{ $type.'_'.$group->group_id }}">
                {{ strlen($group->group_name) > 50 ? trim(substr($group->group_name,0,12)).'..' : $group->group_name }}
        </td>
    </tr>
</table>
@endif
