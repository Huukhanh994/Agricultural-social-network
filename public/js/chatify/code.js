        ...
        if (messenger != 0) {
            $.ajax({
                ...
                success: (data) => {
                    ...
                    messagesContainer.find('.messages').html(data.messages_group);
                    // scroll to bottom
                    scrollBottom(messagesContainer);
                    // remove loading bar
                    NProgress.done();
...
function makeSeen(status) {
    // remove unseen counter for the user from the contacts list
    $('.messenger-list-item[data-contact=' + messenger.split('_')[1] + ']').find('tr>td>b').remove();
    // console.log("SEEN", messenger.split('_')[1]);
    // seen
    $.ajax({
        url: url + '/makeSeen',
        method: 'POST',
 ...
 *-------------------------------------------------------------
 */
function makeSeenGroup(status) {
    // remove unseen counter for the user from the contacts list
    $('.messenger-list-item[data-contact=' + messenger.split('_')[1] + ']').find('tr>td>b').remove();
    // console.log("GROUP", messenger.split('_')[1]);
    // seen
    $.ajax({
        url: url + '/makeSeenGroup',
        method: 'POST',
        data: { '_token': access_token, 'id': messenger.split('_')[1] },
        dataType: 'JSON',
        success: (data) => {
...
function getContactsGroup() {
    $('.listGroupContacts').html(listItemLoading(4));
    $.ajax({
        url: url + '/getGroupContacts',
        method: 'POST',
        data: { '_token': access_token, 'messenger_id': messenger.split('_')[1] },
        dataType: 'JSON',
        success: (data) => {
            $('.listGroupContacts').html('');
            $('.listGroupContacts').html(data.contacts);
            // update data-action required with [responsive design]
            cssMediaQueries();
        },
        error: () => {
